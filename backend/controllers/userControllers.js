...
const loginController = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: user._id }, 'MOWNICA@2021#dk', {
      expiresIn: "1d",
    });
...
const authenticateToken = (req, res, next) => {
   ...
   jwt.verify(token, 'MOWNICA@2021#dk', (err, user) => {
      if (err) {
...
const postCourseController = async (req, res) => {
  ...
  try {
    const { userId, C_educator, C_title, C_categories, C_price, C_description } = req.body;

    // Format sections and attach uploaded content files
    const sections = req.body.sections.map((section, index) => ({
      ...
      S_content: {
        ...
        path: `/uploads/${req.files[index].filename}`
      }
    }));
